...
public class TileTest {
    ...
    public void reveal() throws Exception {
        ...
        assertEquals(state, TileState.REVEALED);
    }
    ...
    public void explode() throws Exception {
        tile.explode();
        TileState state = (TileState) stateField.get(tile);
        assertEquals(state, TileState.EXPLODED);
    }

    @Test
    public void gameOver() throws Exception {
        mineField.set(tile, true);
        stateField.set(tile, TileState.HIDDEN);
        tile.gameOver();
        assertEquals(stateField.get(tile), TileState.HIDDEN_MINE);

        mineField.set(tile, false);
        stateField.set(tile, TileState.FLAGGED);
        tile.gameOver();
        assertEquals(stateField.get(tile), TileState.FALSE_FLAG);
    }
    ...
    public void setFlag() throws Exception {
        tile.setFlag(true);
        assertEquals(TileState.FLAGGED, stateField.get(tile));
        tile.setFlag(false);
        assertEquals(TileState.HIDDEN, stateField.get(tile));
    }

    @Test
    public void isRevealed() throws Exception {
        assertFalse(tile.isRevealed());
        stateField.set(tile, TileState.REVEALED);
